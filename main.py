...
async def on_ready():
  ...
  if firstRun == True:
    ...
    embed = discord.Embed(
        title='''**Подать заявку/Submit an application**''',
        description=f'Чтобы подать заявку в полк нажмите кнопку ниже! \n\nTo apply to the squadron, click the button below!\n\n__**EARLY**__ - для ежедневных полковых боёв / for squadron battles \n __**E4RLY**__ - русско-язычный полк для фарма техники и полковых боев \n__**ENRLY**__ - english-speaking squadron for farming equipment and squadron battles\n __**Друг / Friend**__ - выберите, если хотите присоединиться к серверу как гость / select if you want to join the server as a guest',
        color=embedColor)
