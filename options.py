...
class yesOrNoOption(discord.ui.View):
    ...
    async def yes(self, interaction:discord.Interaction, button: discord.ui.button):
        ...
        embed2 = discord.Embed(title=f'Заявка сохранена и закрыта / The application has been saved and closed', description=f'Заявка была закрыта, сохранена и удалена участником: {author.mention}. / A open ticket has been marked as closed by {author.mention}, it has been logged and deleted.', color=embedColor)
        ...
        embed2.add_field(name="**__Squadron:__**", value=f"{ticketInfo[4]}", inline=True)
        embed2.add_field(name="**__ID автора:__**", value=f"{tauthor.id}", inline=True)
        if dmTicketCopies == False:
            try:
