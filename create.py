...
class TicketCreationMenu(discord.ui.Select):
  ...
  def __init__(self):
    ...
    super().__init__(placeholder="Выберите полк/Choose a squadron...",
                     options=optionsList(),
                     min_values=1,
                     max_values=1)
...
class TicketCreationModal(discord.ui.Modal, title=f"Заявка в полк/Application"):
  ...
  async def on_submit(self, interaction: discord.Interaction):
    ...
    embed1 = discord.Embed(
        title='Новая заявка / Ticket Created',
...
class TicketCreation(discord.ui.View):
  ...
  async def presscreate(self, interaction: discord.Interaction,
    ...
    embed2 = discord.Embed(
          description=
          f'\nПривет, {author.name}! Выберите полк, в который хотите подать заявку. \n \n Hi {author.name}! Select the regiment you want to apply to.\n',
          color=embedColor)
    embed2.set_author(name=f'{author}', icon_url=f'{author.display_avatar}')
      #embed2.set_footer(text=f"{footerOfEmbeds} | {bot.user.id}", icon_url=f'{bot.user.display_avatar}')
